...
1. **Uvoze se sve vrste riječi** iz hrLexa, u svim oblicima (svi padeži, brojevi, rodovi, stupnjevi, konjugirani oblici, klitike): imenice, glagoli (uklj. pomoćne), pridjevi, prilozi, zamjenice (uklj. UD `DET`), brojevi, prijedlozi, veznici, čestice, uzvici i vlastita imena (`PROPN`). PROPN oblici ulaze u igrivu tablicu `rijeci` kao vrsta `vlastito_ime`, a svaka njihova lema daje grupu `vlastito_ime:lema`; jedan prihvaćeni oblik tako troši sve svoje PROPN grupe. Kratice, interpunkcija i strani znakovi i dalje otpadaju; rimski brojevi (`NumForm=Roman`) se isključuju. Riječi s manje od **2 grafema** ne ulaze (ne mogu nikad biti valjan potez).
